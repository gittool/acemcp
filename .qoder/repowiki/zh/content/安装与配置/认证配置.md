...
Web 界面新增了令牌验证功能，允许用户测试 API 令牌的有效性。在配置界面中，增加了 **'Validate Key'** 按钮，用户可点击该按钮对当前或待更新的令牌进行验证。

#### 验证流程

1. 用户点击 **'Validate Key'** 按钮
2. 前端通过 `/api/validate-token` 端点发送验证请求
3. 系统使用提供的 `BASE_URL` 和 `TOKEN` 向外部 API 发送测试请求
4. 根据响应结果返回验证状态
...
- **超时错误**：无法连接到 API 服务
- **连接错误**：BASE_URL 配置错误或网络问题
- **有效令牌**：成功通过验证

#### 响应状态显示

验证结果会以不同颜色和消息显示在界面中：
- **绿色**：令牌有效（`Token is valid and working!`）
- **红色**：验证失败，显示具体错误原因
...
participant Server as "服务器"
participant API as "外部API服务"
User->>WebUI : 点击 'Validate Key' 按钮
WebUI->>Server : POST /api/validate-token
Server->>API : 测试请求 (空批量上传)
API-->>Server : 返回状态码
Server-->>WebUI : 返回验证结果
WebUI-->>User : 显示验证状态 (成功/失败)
```

**Diagram sources**
- [app.py](file://src/acemcp/web/app.py#L140-L193)
- [index.html](file://src/acemcp/web/templates/index.html#L59-L65)

**Section sources**
- [app.py](file://src/acemcp/web/app.py#L140-L193)
- [index.html](file://src/acemcp/web/templates/index.html#L51-L98)
...
新增的 `/api/validate-token` 端点实现了专门的错误处理机制，能够检测和报告多种验证失败场景：

- **401 Unauthorized**：返回 "Token is invalid or expired (401 Unauthorized)"
- **403 Forbidden**：返回 "Token does not have permission (403 Forbidden)"
- **超时**：返回 "Request timed out. Please check your BASE_URL"
- **连接错误**：返回 "Cannot connect to the API. Please check your BASE_URL"
- **其他错误**：返回具体的异常信息
...
- 系统实施了严格的配置验证和错误处理机制
- 安全性是配置管理的重要考虑因素
- 新增了令牌验证功能，可通过 `/api/validate-token` 端点和 Web 界面的 'Validate Key' 按钮测试令牌有效性
