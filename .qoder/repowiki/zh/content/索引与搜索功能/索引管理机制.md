...
<cite>
**本文档引用的文件**
- [manager.py](file://src/acemcp/index/manager.py) - *更新了HTTP客户端共享实例和资源管理*
- [search_context.py](file://src/acemcp/tools/search_context.py) - *新增了共享实例管理和关闭逻辑*
- [server.py](file://src/acemcp/server.py) - *在服务器关闭时调用资源清理*
</cite>

## 更新摘要
**变更内容**
- 新增了HTTP客户端共享实例和资源管理说明
- 更新了核心组件章节，增加了`_get_client()`方法和`close()`方法的说明
- 在使用示例中增加了资源管理的最佳实践
- 更新了所有相关章节的源文件引用
...
新增了HTTP客户端共享实例和资源管理功能，通过`_get_client()`方法获取共享的httpx.AsyncClient实例，并通过`close()`方法进行资源清理。这种设计提高了资源效率并确保了HTTP连接的正确生命周期管理。
...
## 使用示例
以下是`IndexManager`的初始化和调用流程示例，包括HTTP客户端共享实例和资源管理的最佳实践：
...
UseResult --> CloseClient["调用close()关闭HTTP客户端"]
CloseClient --> End([流程结束])
```

**Diagram sources**
- [manager.py](file://src/acemcp/index/manager.py#L331-L466)
- [search_context.py](file://src/acemcp/tools/search_context.py#L14-L37)
- [server.py](file://src/acemcp/server.py#L137-L139)
